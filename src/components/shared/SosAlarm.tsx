...
export const SosAlarm = () => {
  ...
  return (
    <div className="fixed top-6 right-6 z-[9999] w-80 md:w-96 flex flex-col gap-2 animate-in slide-in-from-right duration-300">
      ...
      {/* Notificación Push */}
      <div className="bg-zinc-900 border border-red-500/50 rounded-xl p-4 shadow-2xl flex gap-4 relative overflow-hidden cursor-pointer hover:bg-zinc-800 transition-colors">
        ...
        <div className="flex-1">
          <h4 className="text-white font-bold text-sm">SONA Empordà</h4>
          <p className="text-white/70 text-xs mt-1">🚨 Nueva Alerta SOS: {activeAlert.title}</p>
        </div>
        <button onClick={(e) => { e.stopPropagation(); setActiveAlert(null); }} className="text-white/40 hover:text-white h-fit">
